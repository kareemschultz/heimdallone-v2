...
function TemplateDetailPage() {
	...
	const template = useQuery(
		orpc.onboarding.templates.getById.queryOptions({ input: { id } })
	);
	...
	const archiveMutation = useMutation({
		mutationFn: () => client.onboarding.templates.archive({ id }),
		onSuccess: () => {
			toast.success("Template archived.");
			invalidate();
			navigate({ to: "/app/onboarding/templates" });
		},
		onError: (err: Error) => {
	...
	return (
		<div className="page">
			...
			{showEdit && t && (
				<TemplateFormDialog
					initial={{ name: t.name, description: t.description ?? "" }}
					mode="edit"
