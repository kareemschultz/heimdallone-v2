...
> ledger-adjustment status, gated to payroll admins; the issued payslip above stays the immutable
> original. The payroll run confirm/confirmed views (`run.tsx`) surface the pinned **statutory rule
> version** ("resolved by pay date and pinned to this run"). UI-only (reads `payroll.corrections.list` +
> `runs.getById.ruleVersionLabel`) → audit stays 161/21. Web tsc baseline unchanged (5; changed files
> clean). Browser verification deferred (no running app/auth in this session). Remaining: a "run a
> correction" action button (preview→apply) in the payslip screen.
