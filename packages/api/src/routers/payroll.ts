...
/**
 * Roll a pay period forward to the next consecutive period of the same cadence.
 * Calendar-based frequencies (monthly/semi-monthly/annual) advance by month/year;
 * fixed-length frequencies (weekly/fortnightly and any other) advance by the
 * period's own day-length so periods stay back-to-back with no gap or overlap.
 */
function computeNextPeriod(p: {
	...
	payDate: Date | null;
	frequency: string;
}): { startDate: Date; endDate: Date; payDate: Date | null } {
	const freq = p.frequency.toLowerCase();
	if (freq === "monthly" || freq === "semi_monthly" || freq === "semimonthly") {
		const step = freq === "monthly" ? 1 : 0;
		// Semi-monthly: advance by the period length (≈ half a month) to stay
		// back-to-back; monthly: advance one calendar month.
		if (step === 1) {
			return {
				startDate: addUtcMonths(p.startDate, 1),
				endDate: addUtcMonths(p.endDate, 1),
				payDate: p.payDate ? addUtcMonths(p.payDate, 1) : null,
			};
		}
	}
	if (freq === "annual" || freq === "yearly") {
		return {
			startDate: addUtcMonths(p.startDate, 12),
			endDate: addUtcMonths(p.endDate, 12),
			payDate: p.payDate ? addUtcMonths(p.payDate, 12) : null,
		};
	}
	// weekly / fortnightly / semi-monthly / default: next back-to-back period of
	// the same day-length.
	const lengthDays =
		Math.round(
			(p.endDate.getTime() - p.startDate.getTime()) / (24 * 60 * 60 * 1000)
		) + 1;
	const startDate = addUtcDays(p.endDate, 1);
	const endDate = addUtcDays(startDate, lengthDays - 1);
	return {
		startDate,
		endDate,
		payDate: p.payDate ? addUtcDays(p.payDate, lengthDays) : null,
	};
}
...
const payPeriodsClose = authorizedProcedure("payroll", "update")
	...
	.handler(async ({ context, input }) => {
		...
		// Tenant policy: when auto-generate is enabled, opening the next pay period
		// on close removes the manual step. Default off → unchanged for other tenants.
		const nextPeriodId = await maybeOpenNextPeriod(
			orgId(context),
			actorId(context),
			existing
		);

		return { id: input.id, nextPeriodId };
	});

/**
 * If the org has auto-generate enabled, create the next consecutive pay period
 * (same cadence) when one is closed. Idempotent: skips if a period with the
 * computed start date already exists. Returns the new period id, or null.
 */
async function maybeOpenNextPeriod(
	organizationId: string,
	createdBy: string,
	closed: typeof payPeriod.$inferSelect
): Promise<string | null> {
	const [settings] = await db
		.select({ enabled: payrollSetting.autoGenerateEnabled })
		.from(payrollSetting)
	...
	const next = computeNextPeriod({
		...
		payDate: closed.payDate,
		frequency: closed.frequency,
	});
	// Idempotency: don't duplicate a period that already starts on that day.
	const [dupe] = await db
		.select({ id: payPeriod.id })
		.from(payPeriod)
		.where(
			and(
				eq(payPeriod.organizationId, organizationId),
				eq(payPeriod.startDate, next.startDate)
			)
		)
		.limit(1);
	if (dupe) {
	...
	await db.insert(payPeriod).values({
		id,
		organizationId,
		name: `Pay period ${next.startDate.toISOString().slice(0, 10)} – ${next.endDate.toISOString().slice(0, 10)}`,
		startDate: next.startDate,
		endDate: next.endDate,
		payDate: next.payDate,
		frequency: closed.frequency,
		workingDays: closed.workingDays,
		expectedHours: closed.expectedHours,
		status: "open",
	});
	await createAuditEvent(db as never, {
		organizationId,
		...
		actorId: createdBy,
		changes: [
			{ field: "autoGeneratedFrom", oldValue: null, newValue: closed.id },
		],
	});
	return id;
}
