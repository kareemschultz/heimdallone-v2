...
- Recruitment + Onboarding (Phase 9): **In progress**
  ...
  - 9D remaining: interview row actions, pipeline quick-view drawer (Decision B).
...
- Next: Phase 9D — interview row actions, pipeline quick-view drawer (Decision B); then 9E–9G Onboarding, 9H candidate→employee conversion, 9I QA + API denormalization (candidate/opening display fields in interviews.list/offers.list/applications.list) + server-verify departmentId/jobPositionId/recruiterUserId before exposing in job forms.
