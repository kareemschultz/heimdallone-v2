...
- **Assets (Phase 12): ACTIVE.** Spec `docs/architecture/assets-implementation-plan.md`. **12B DB ✅** — `schema/assets.ts` (4 tables asset_category/asset/asset_assignment/asset_request + 3 enums asset_status/asset_return_condition/asset_request_status), migration **0014_fat_shiver_man.sql** applied; partial-unique invariants `asset_org_tracking_uq`, `asset_category_org_name_uq`, **`asset_assignment_open_uq`** (≤1 open assignment/asset); `asset.currentAssigneeId` denormalised cache; `purchaseCost` numeric (12C must redact non-finance). `scripts/seed-assets.ts` idempotent: 5 categories/10 assets/8 assignments (5 open+3 returned)/4 requests. Invariants verified (one-open + trackingId-unique reject on insert); seed idempotent ×2; gates check-types 3/3, build, lint 223/1/2, audit 73/10 (no router yet). Doc `docs/implementation/assets-db-setup.md`. No offboarding coupling this phase. **Next: 12C Assets oRPC API** (categories/assets/assignments/requests; two-layer authz; transactional assign/return; cost redaction; confirm whether employee role needs `asset:request` AC action before self-service — the offboarding markUploaded trap).
