...
- Recruitment + Onboarding (Phase 9): **In progress**
  ...
  - 9D **CHECKPOINT 3** ✅: Interviews list (status filter chips, client-side join of candidate+opening, status badges, EmptyState) — live-verified (4 seeded interviews render + join + filter, 0 console errors). Per-row interview actions (schedule/reschedule/complete/cancel/no-show/feedback) deferred to a follow-up.
  - 9D remaining: interview row actions, Offers list+detail, Reports, full Candidate detail (5 tabs), Job create/edit forms.
...
- Next: Phase 9D checkpoint 4 — Offers list+detail (#92), then Reports (#93), interview row actions, Candidate detail (#95), Job forms (#96).
