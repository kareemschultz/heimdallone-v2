...
- **Helpdesk / Requests (Phase 13): ACTIVE — 13A spec ✅, 13B DB next.** Plan `docs/architecture/helpdesk-requests-implementation-plan.md`. **CENTRAL GUARDRAIL (user directive): Helpdesk is the request/ticket LAYER that LINKS to Assets/Payroll/Leave/Attendance/Offboarding — NEVER a second copy of their logic.** Schema expresses it via read-only nullable link cols (linkedAssetId/linkedPayslipId/linkedLeaveRequestId/linkedOffboardingCaseId + generic linkedEntityType/Id); helpdesk reads/deep-links but never mutates those modules (no pay change; blockers stay in payroll_issue, procurement in asset_request). **Reuses the EXISTING `ticket` AC** (create/read/update/assign/resolve/close) — employee already holds `ticket:create` so NO new action needed (unlike assets' asset:request); optional `ticket:approve` for approvals (open Q). helpdesk_agent role already exists. **MVP = 3 tables** (helpdesk_category, helpdesk_request [category/priority/status/assignee/SLA dues/single-step-approval cols/link cols], helpdesk_request_comment [isInternal redaction]); deferred sla_policy/status_history/attachment/approval-table/KB. Status New→Open→In-progress→Waiting(employee/approval)→Resolved→Closed+Cancelled; priority low/normal/high/urgent; SLA states on_track/due_soon/overdue/breached/not_applicable; 10 seeded categories. Router `helpdesk`; UI HelpdeskTabs + /app/helpdesk/* (DELETE flat helpdesk.tsx stub = route-shadow #4; sidebar entry safe now post-12E route.tsx a11y cleanup). Benchmarked Zendesk/Freshdesk/Jira-SM/Frappe-ERPNext/GLPI/ServiceNow/Horilla. 13A→13H. Gates (docs-only) check-types 3/3, build, lint 212 unchanged, audit 86/12. **Next: 13B DB schema + seed.**
