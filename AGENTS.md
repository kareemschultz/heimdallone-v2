...
- **Biometric + Geofencing (Phase 11): ACTIVE.** 11A spec ✅ (`docs/architecture/biometric-geofencing-implementation-plan.md`); **11B DB schema + migration + seed ✅** — `packages/db/src/schema/biometric.ts` (8 tables + 13 enums), migration `0011_early_lady_vermin.sql` applied (additive only), `scripts/seed-biometric.ts` (re-runnable). Punch idempotency + privacy (no biometric templates, no plaintext secrets) verified in-seed. Reuses attendance_event/record/correction. **Next: 11C device sync/import API + punch processor** (adds the new AC resources `attendance_device`/`attendance_punch`/`geofence`/`attendance_exception` + RBAC helpers).
