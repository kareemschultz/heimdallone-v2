...
- **Biometric + Geofencing (Phase 11): ACTIVE.** 11A spec ✅; 11B DB ✅ (`schema/biometric.ts`, 8 tables + enums, migrations `0011`/`0012`); **11C sync/import API + processor ✅** — `routers/biometric.ts` (devices/mappings/syncRuns/punches/processor/ingest/geofences/assignments/checkIns/exceptions). **Multi-vendor adapter/provider model** (`utils/attendance-adapters.ts`): ZKTeco TCP/ADMS + NGTeco cloud/app/K-series + generic CSV/Excel/USB/API; live ZKTeco/NGTeco-cloud are `planned` (no faked sync). Source-agnostic idempotent processor (`utils/biometric-processor.ts`) bridges staged punches → `attendance_event` → `recalculateRecord` (extracted to `utils/attendance-recalc.ts`). Public `ingest.submit` = device API-key auth. 4 AC resources + 6 RBAC helpers. **Privacy:** no biometric templates ingested/stored; device secrets never returned. audit:permissions 73/10. **Next: 11D biometric devices UI.**
